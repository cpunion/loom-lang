...
`fib_recursive` 命中 compiler-private checked-entry/assumed-body 路径。这个分析不依赖函数名，但仅接受纯、同步、非泛型、无 receiver/合同/witness、单个 immutable `Int -> Int` 且 direct self-recursive 的保守形状。编译器从 0 开始对稠密输入做有界精确求值，最多到 128；只有每条递归边严格降低且所有整数运算/递归 site 都被成功域覆盖才建立证明。当前 Fibonacci 安全域是 `0..92`；checked entry 的 unsigned guard 使负数进入 slow path，域内 assumed body 是无 context/status/overflow helper 的纯 `i64` 递归。基准中 `run` 对 size 的完整推导 range 是 `0..45`，因此可直接调用 assumed body；不能完整证明的 call site、`fibonacci(93)` 和其他域外输入仍走 checked overflow 路径。
...
这些 case 固定的是当前 compiler-private 热路，不是统一容器/record ABI。`NativeLayout` catalog 已统一 scalar 与 direct-primitive POD record 的物理分类；private callable selector 支持 scalar 以及 POD result/`mut self` InOut，并为 universal、native checked 和可选 assumed body 分开计算 requirement。universal fallback 是当前不兼容 aggregate 边界的语义实现，不是历史兼容层。普通 POD 参数、readonly receiver 和合同/invariant checked-entry/private-body 仍是 P0；List/managed generic layout、统一 clone/trace/drop plan 与 machine-instance identity 仍待扩展。每个 case 内五种语言继续使用完全相同的规模；若最快实现接近进程启动时间，其相对倍数只能视为下界。
