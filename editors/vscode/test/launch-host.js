...
async function main() {
  ...
  try {
    ...
    const child = spawn(process.env.VSCODE_EXECUTABLE || (process.platform === 'win32' ? 'Code.exe' : 'code'), [
      ...
      LOOM_EDITOR_HOST_CHECKOUT: checkout ? '1' : '',
    } });
    ...
    console.log(checkout ? 'VS Code checkout settings, compiler/std/example diagnostics, and unsaved repair passed.' :
      'VS Code activation, unsaved diagnostics, hover, definition, name/member/qualified/import completion, formatting/save, test, and run passed.');
  } catch (error) {
