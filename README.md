...
当前 native runtime ABI 总版本是 v7，精确 identity 为 `loom-value-v2/layout-v1/text-v1/wait-v1/task-v2/runtime-v1/gc-v7/shadow-stack-v1/witness-v1/int-list-v1/stdlib-v4`。LLVM 同步函数已经发射 safepoint-specific precise shadow-stack bitmap；managed allocation slowpath 可在分配前 collection，Rust helper 以 `RuntimeRootScope`/`NodeStream` 和 stable full-`Value` output 保持中间值可重定位。普通值 clone/node build 使用 Rust runtime 的非递归 work stack，不再生成 LLVM module-local 递归 helper。coroutine suspension 的 live-local metadata 来自正常、取消和 `defer` cleanup edge 共用的结构化 CFG backward liveness，并由 MIR validator 独立重算；Executor 只负责 task/join 与 wait 调度，不承载同步 root bitmap 或 runtime helper root scope。

尚未完成的是跨函数 POD record concrete ABI、managed generic/container 的 layout-driven calling convention、统一 clone/trace/drop plan 与更完整的 coroutine/`dyn` layout specialization，不把这些优化误报为当前闭环。
...
基础跨语言性能基准另由 `loom-benchmark` 对照 Loom release LLVM、Go、Rust、C 与 C++。v1 在相同动态输入和 checksum 下记录一次优化构建（Loom 禁用项目缓存，其他工具保留各自标准工具链缓存策略）、native runtime 原始样本及 artifact size。当前 native 已落地 closed-world `FAULT`/`ALLOC`/`EXECUTOR` runtime-requirement 图，以及 `Unit -> i1`、`Bool -> i1`、`Int -> i64`、`Float -> double` 的 primitive private ABI；pure/no-fault body 没有隐藏 context/status，fallible checked-`Int`/fib 等 body 保留 status/context。eligible 函数内 POD record 在入口栈节点上更新并由 release SROA 提升；只有 whole-value copy/call/return 边界才物化独立 managed chain，保持深复制。eligible 同步不逃逸局部 `List[Int]` 使用 compiler-private contiguous `{data, len, cap}`：canonical append loop 以 SSA 携带 header，只在 reserve 成功后重载 data/cap，并在元素写入后立即提交 len；对同一稳定长度的零起点完整 scan，closed-world 证明可删除逐元素 upper-bound 与不可达 `None` edge。terminal fault/status edge 带明确的 unlikely 权重，fault sink 标记为 cold/noinline，普通业务分支不因此带偏置。append/exact 子证明失败时 List 保留 checked generic-native 路径；只有 native-storage eligibility 失败才回退当前 universal `Value` lowering。

这些都是 closed-world/compiler-private 快路：四种 primitive scalar private ABI 已可跨函数，record/List 优化当前仍局限在函数内。`NativeLayout` catalog 现在同时描述 scalar 与无 invariant、单态、直接 primitive-field 的 POD record；record 栈存储候选已复用该分类，但 callable `NativeSignatureShape` 仍只放行 emitter 已完整支持的 primitive scalar，不会因“物理上是 POD”提前删除分配。List/其他布局、统一 clone/trace/drop plan、完整 machine-instance graph 与跨函数 POD ABI 仍待扩展。generic/`Text` direct ABI 以及一般 layout-driven container/coroutine/`dyn` layout 也在后续阶段。warm/incremental build、peak RSS 和固定机器趋势属于下一层证据；繁忙共享主机上的手动 wall-time 只可作诊断，不能写成稳定性能结论。它与 `loom-quality` 的回归上界分开，不把一次共享机器结果解释为语言排名；完整方法、运行命令、证据边界和 P0–P2 后续计划见[质量与受控证据](docs/09-quality-and-controlled-evidence.md#31-跨语言基础基准)。
