...
这些都是 closed-world/compiler-private 快路：`Int` private ABI 已可跨函数，record/List 优化当前仍局限在函数内。首个仅覆盖 scalar `Int` 的 `NativeLayout`/`NativeSignatureShape` catalog 已接入，并成为 requirement graph 与 emitter 的共同 private-ABI selector；POD record/List/其他布局、统一 clone/trace/drop plan、完整 machine-instance graph 与跨函数 POD ABI 仍待扩展。generic/`Text` direct ABI 以及一般 layout-driven container/coroutine/`dyn` layout 也在后续阶段。warm/incremental build、peak RSS 和固定机器趋势属于下一层证据；繁忙共享主机上的手动 wall-time 只可作诊断，不能写成稳定性能结论。它与 `loom-quality` 的回归上界分开，不把一次共享机器结果解释为语言排名；完整方法、运行命令、证据边界和 P0–P2 后续计划见[质量与受控证据](docs/09-quality-and-controlled-evidence.md#31-跨语言基础基准)。
