...
const LLVM_OBJECT_CACHE_DOMAIN: &str = "loom-llvm-object-cache-v9";
...
mod tests {
    ...
    fn llvm_object_cache_domain_is_pinned() {
        assert_eq!(super::LLVM_OBJECT_CACHE_DOMAIN, "loom-llvm-object-cache-v9");
    }
    ...
    fn checked_mir_cache_identity_pins_interpreted_artifact_version() {
        assert_eq!(loom_mir::INTERPRETED_ARTIFACT_VERSION, 20);
        let context = super::cache_context(loom_mir::LOOM_LANGUAGE_VERSION);
        assert!(
            context
                .frontend_identity
                .ends_with("/loom.interpreted-mir-20"),
            "{}",
            context.frontend_identity
        );
    }
