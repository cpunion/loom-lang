...
fn assert_compiler_owned_overlays_are_ignored(
    ...
    host.set_overlay(json_path, "pub fn forged_json() Int { 999 }\n")
        .expect("install hostile std.json overlay");
    host.set_overlay(log_path, "pub fn debug(message Text) { discard message }\n")
    ...
    host.set_overlay(resource_path, "pub concept ForgedResourceProtocol {}\n")
...
fn compiler_std_sources_have_protected_authority() {
    ...
    assert!(
        std_sources
            .iter()
            .any(|source| source.relative_path().ends_with("json/json.loom")),
        "{std_sources:#?}"
    );
    assert!(
        std_sources
            .iter()
            .any(|source| source.relative_path().ends_with("log/log.loom")),
        "{std_sources:#?}"
    );
    assert!(
        std_sources
            .iter()
            .any(|source| source.relative_path().ends_with("process/process.loom")),
        "{std_sources:#?}"
    );
    assert!(
        std_sources
            .iter()
            .any(|source| source.relative_path().ends_with("resource/resource.loom")),
        "{std_sources:#?}"
    );
    ...
    let resource_path = std_sources
        .iter()
        .find(|source| source.relative_path().ends_with("resource/resource.loom"))
        .expect("compiler-owned std.resource source")
        .absolute_path()
        .to_path_buf();
    let json_path = std_sources
        .iter()
        .find(|source| source.relative_path().ends_with("json/json.loom"))
        .expect("compiler-owned std.json source")
        .absolute_path()
        .to_path_buf();
    let log_path = std_sources
        .iter()
        .find(|source| source.relative_path().ends_with("log/log.loom"))
    ...
    let std_source = std_sources
        .iter()
        .copied()
        .find(|source| source.relative_path().ends_with("int/int.loom"))
...
fn source_backed_std_functions_lower_to_direct_calls_and_run() {
    ...
    let emit = program
        .functions
        .iter()
        .find(|function| function.name == "standalone.emit")
...
#[test]
fn std_module_name_and_dependency_alias_are_reserved() {
    ...
    assert!(error.to_string().contains("module name `std` is reserved"));
    ...
    adjacent_namespace.write("main.loom", "pub fn main() {}\n");
...
fn human_diagnostics_use_scalar_columns_and_keep_machine_records_stable() {
    ...
    assert!(human.contains("2 |     let 价格 = 1"), "{human}");
...
fn discovery_is_recursive_sorted_and_ignores_git_and_target() {
    let project = TestProject::new();
    project.write("z.loom", "");
    project.write("src/a.loom", "");
    project.write("target/generated.loom", "");
    project.write("nested/.git/hidden.loom", "");
    project.write("src/not-loom.txt", "");
...
#[test]
fn manifest_resolves_directory_packages_and_targets() {
    let project = TestProject::new();
    project.write(
        "utility/loom.toml",
        "schema = 2\n\n[module]\nname = \"utility\"\nversion = \"1.2.0\"\n",
    ...
    let host =
        AnalysisHost::new_with_options(project.root.join("application"), &test_project_options())
            .expect("open manifest project");
    ...
    assert_eq!(
        paths,
        [
            "deps/utility@1.2.0/math/math.loom",
            "main.loom",
            "main_test.loom"
        ]
    );
...
fn portable_library_is_a_consumable_versioned_dependency() {
    ...
    malformed_source["sources"][0]["text"] = serde_json::json!("fn {");
    ...
    assert!(decoded.interfaces().iter().any(|interface| {
        interface.module.ends_with("::utility.nested") && interface.files == ["nested/nested.loom"]
    }));
    ...
    let dependency = snapshot
        ...
        .find(|source| {
            source
                ...
                && source.relative_path().ends_with("math/math.loom")
        })
    ...
    assert!(
        std_sources
            .iter()
            .any(|source| source.relative_path().ends_with("int/int.loom")),
        "{std_sources:#?}"
    );
    assert!(
        std_sources
...
fn portable_library_carries_source_instead_of_process_local_proofs() {
    ...
    fs::remove_file(project.root.join("main.loom")).expect("remove producer checkout");
...
fn portable_library_lock_closes_over_transitive_source_content() {
    ...
    project.write("leaf/lib.loom", "pub fn base() Int { 41 }\n");
...
fn package_imports_are_limited_to_direct_dependencies() {
    ...
    project.write("leaf/lib.loom", "pub fn value() Int { 1 }\n");
    project.write(
        "middle/loom.toml",
        "schema = 2\n[module]\nname = \"middle\"\nversion = \"1.0.0\"\n[dependencies]\nleaf = { path = \"../leaf\" }\n",
    );
    project.write("middle/lib.loom", "");
    project.write(
        "root/loom.toml",
        "schema = 2\n[module]\nname = \"root\"\nversion = \"1.0.0\"\n[dependencies]\nmiddle = { path = \"../middle\" }\n",
    );
...
#[test]
fn source_directories_define_package_names() {
    let project = TestProject::new();
    project.write(
        "dependency/loom.toml",
        "schema = 2\n[module]\nname = \"dependency\"\nversion = \"1.0.0\"\n",
    );
    project.write("dependency/nested/lib.loom", "pub fn hijack() {}\n");
    project.write(
        "application/loom.toml",
    ...
    project.write(
        "application/main.loom",
        "import dependency.nested.hijack\n\npub fn main() {\n    hijack()\n}\n",
    );

    let snapshot = AnalysisHost::new(project.root.join("application"))
        .expect("open directory-package graph")
        .snapshot()
        .expect("analyze directory packages");
    assert!(!snapshot.has_errors(), "{:#?}", snapshot.diagnostics());
    assert!(
        snapshot
            .module_interfaces()
            .iter()
            .any(|interface| { interface.module.ends_with("::dependency.nested") })
    );
}
...
fn manifest_fails_closed_on_version_mismatch_and_dependency_cycle() {
    ...
    mismatch.write("dependency/lib.loom", "");
    mismatch.write(
        "root/loom.toml",
        "schema = 2\n[module]\nname = \"root\"\nversion = \"0.1.0\"\n[dependencies]\ndependency = { path = \"../dependency\", version = \"^1\" }\n",
    );
    mismatch.write("root/main.loom", "");
    ...
    cycle.write("a/a.loom", "");
    cycle.write(
        "b/loom.toml",
        "schema = 2\n[module]\nname = \"b\"\nversion = \"0.1.0\"\n[dependencies]\na = { path = \"../a\" }\n",
    );
    cycle.write("b/b.loom", "");
...
#[test]
fn lockfile_requires_module_entries() {
    let project = TestProject::new();
    project.write(
        "loom.toml",
        "schema = 2\n[module]\nname = \"sample\"\nversion = \"1.0.0\"\n",
    ...
    project.write("lib.loom", "");
    project.write("loom.lock", "schema = 2\n");

    let error = ProjectGraph::load(&project.root).expect_err("module entries are required");
    assert_eq!(error.code(), "ProjectLoadFailed");
    assert!(
        error.to_string().contains("invalid lockfile")
            && error.to_string().contains("missing field `module`"),
        "{error}"
    );
}
...
fn registry_features_and_lockfile_form_a_reproducible_graph() {
    ...
    project.write("application/main.loom", "pub fn main() {\n}\n");
...
fn persistent_cache_is_relocatable_and_corruption_is_a_safe_miss() {
    ...
    first.write("main.loom", "pub fn main() {\n    assert true\n}\n");
...
fn typed_hir_queries_reuse_unmodified_modules() {
    ...
    let host = AnalysisHost::new_with_options(&project.root, &test_project_options())
        .expect("open incremental project");
...
fn many_modules_and_call_edges_close_the_checked_mir_pipeline() {
    ...
    for index in 0..MODULES {
        let source = if index + 1 == MODULES {
            format!("pub fn f{index}(value Int) Int {{\n    value\n}}\n")
        } else {
            let next = index + 1;
            format!(
        ...
        project.write(&format!("m{index}/functions.loom"), &source);
    }
    project.write(
        "main.loom",
        &format!(
    ...
    assert!(program.exports.contains_key("stress.main"));
}

#[test]
fn snapshot_assigns_file_ids_by_stable_relative_path_and_builds_executable_mir() {
    let project = TestProject::new();
    project.write("b.loom", "fn b() {\n}\n");
    project.write("a.loom", "fn a() {\n}\n");
...
fn source_map_converts_utf16_positions_without_splitting_scalars() {
    let project = TestProject::new();
    project.write("unicode.loom", "fn 示例() {}\n");
    let snapshot = AnalysisHost::new(&project.root)
        .expect("open host")
        .snapshot()
        .expect("snapshot");
...
"#,
    ...
    project.write("io_test.loom", &source);
