...
#[test]
fn identity_schema_is_pinned_after_typed_proof_rechecks() {
    assert_eq!(ARTIFACT_IDENTITY_SCHEMA, 19);
}
...
fn complete_tuple_semantics_are_dump_and_artifact_identity_inputs() {
    ...
    assert_eq!(
        ARTIFACT_IDENTITY_SCHEMA, 19,
        "typed proof replay advances the identity schema after managed Lists"
    );
}
