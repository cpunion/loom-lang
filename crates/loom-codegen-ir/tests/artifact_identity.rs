...
use loom_codegen_ir::{
    ARTIFACT_IDENTITY_SCHEMA, ArtifactRootRequest, BlockTarget, CanonicalTypeCatalog,
...
fn identity_schema_is_pinned_for_the_current_lcir_contract() {
    assert_eq!(ARTIFACT_IDENTITY_SCHEMA, 57);
}
...
fn identity_is_brand_independent_and_repeatable() {
    ...
    assert!(
        ...
        "artifact schema 57 changes checked meaning with the LCIR text wire: {identity}"
    );
