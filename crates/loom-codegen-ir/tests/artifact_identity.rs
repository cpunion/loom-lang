...
#[test]
fn identity_schema_is_pinned_after_typed_resource_cleanup() {
    assert_eq!(ARTIFACT_IDENTITY_SCHEMA, 14);
}
...
fn complete_tuple_semantics_are_dump_and_artifact_identity_inputs() {
    ...
    assert_eq!(
        ARTIFACT_IDENTITY_SCHEMA, 14,
        "typed resource cleanup advances the identity schema after managed Text products"
    );
}
