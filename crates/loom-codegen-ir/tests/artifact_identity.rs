...
#[test]
fn identity_schema_is_pinned_after_managed_sum_rooting() {
    assert_eq!(ARTIFACT_IDENTITY_SCHEMA, 15);
}
...
fn complete_tuple_semantics_are_dump_and_artifact_identity_inputs() {
    ...
    assert_eq!(
        ARTIFACT_IDENTITY_SCHEMA, 15,
        "managed Text sums advance the identity schema after typed resource cleanup"
    );
}
