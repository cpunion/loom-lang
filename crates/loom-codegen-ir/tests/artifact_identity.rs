...
#[test]
fn identity_schema_is_pinned_after_typed_text_selection() {
    assert_eq!(ARTIFACT_IDENTITY_SCHEMA, 16);
}
...
fn complete_tuple_semantics_are_dump_and_artifact_identity_inputs() {
    ...
    assert_eq!(
        ARTIFACT_IDENTITY_SCHEMA, 16,
        "typed Text selection advances the identity schema after managed Text sums"
    );
}
