...
#[test]
fn identity_schema_is_pinned_after_typed_scalar_builtins() {
    assert_eq!(ARTIFACT_IDENTITY_SCHEMA, 20);
}
...
fn complete_tuple_semantics_are_dump_and_artifact_identity_inputs() {
    ...
    assert_eq!(
        ARTIFACT_IDENTITY_SCHEMA, 20,
        "typed scalar builtin instructions advance the identity schema after proof replay"
    );
}
