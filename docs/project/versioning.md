...
| Persistent compiler cache | schema `25` |
| Compilation-cache domain | `loom-compilation-cache-v25` |
...
Persistent compiler-cache schema 25 and its matching domain replace the three
public-name Path builtin call identities with ordinary `std.path` inherent
method definitions. Only those exact source bodies may retain the private
`PathFromText`, `PathAsText`, and `PathJoin` leaves. The leaves lower to the
existing MIR operations, so interpreted MIR, LCIR, runtime, and standard-
library ABI versions do not change. The standard-library content digest also
invalidates dependent native objects.
