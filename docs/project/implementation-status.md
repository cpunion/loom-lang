...
| Code generation IR foundation | Production native preparation attempts one atomic whole-artifact direct MIR-to-LCIR lowering. The current slice covers primitives, literal-only or concat-produced direct `Text` on 64-bit targets, structural tuples and acyclic closed records including managed Text leaves, fresh-source proven record invariants, established monomorphic refined values, eligible concrete closed enums, and bounded concrete direct generic instances. Concrete static concept calls become ordinary direct typed calls. Dynamic Text concat and Text-bearing products select one artifact-wide managed Text provenance mode; products remain unboxed SSA, while exact live-after projected leaves use the typed shadow stack without a universal value or executor. Tuple/product and pointer-free closed-sum SSA, bounded nested places, functional mutation/writeback, exhaustive match DAGs, canonical typed assertions, lexical `defer`, and `scoped` StaticConcept/File/Socket disposal remain part of the checked boundary. Cleanup suffixes are static LIFO CFG on normal, return, and fault exits; no runtime cleanup stack or synchronous executor is used. Complete artifacts use independently checked LCIR and its typed LLVM emitter; only reachable `Unsupported` input selects the complete legacy graph. Dynamic concept dispatch, portable proof `Recheck`, runtime-checked construction, source contracts, `Text.get`, Text inside enums or transparent/refined carriers, managed lists, other managed values, recursive or open sums, async, and protected or managed projected inout remain atomic fallback. |
