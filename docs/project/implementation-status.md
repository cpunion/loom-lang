...
| `fixtures/lcir-async-writeback` | synchronous functional inout calls inside typed coroutines, normal and fault receiver writeback, fault writeback before lexical cleanup, by-value dynamic View parameters under mutable dispatch, recursive unique-witness `dyn` erasure across async parameters, results, and nested frames, multi-witness finite dynamic calls outside suspension rows, interpreter/legacy/typed differentials, Linux/MSVC objects, and real `check/build/test/run` commands |
