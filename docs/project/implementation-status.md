...
| Code generation IR foundation | Production native preparation attempts one atomic whole-artifact direct MIR-to-LCIR lowering. The current slice covers primitives, literal-only or concat-produced direct `Text` on 64-bit targets, structural tuples, acyclic closed records, fresh-source proven record invariants, established monomorphic refined values, eligible concrete closed enums, and bounded concrete direct generic instances. Concrete static concept calls become ordinary direct typed calls. Dynamic Text concat uses one artifact-wide managed pointer representation, a staging runtime helper, and exact live-after typed shadow-root maps without a universal value or executor. Tuple/product and closed-sum SSA, bounded nested places, functional mutation/writeback, exhaustive match DAGs, and canonical typed fault metadata remain part of the checked boundary. Complete artifacts use independently checked LCIR and its typed LLVM emitter; only reachable `Unsupported` input selects the complete legacy graph. Dynamic concept dispatch, portable proof `Recheck`, runtime-checked construction, `Text.get`, text in aggregates, other managed values, recursive or open sums, cleanup/async, and protected or managed projected inout remain atomic fallback. |
