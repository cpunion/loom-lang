...
| Code generation IR foundation | Production native preparation attempts one atomic whole-artifact direct MIR-to-LCIR lowering. The current slice covers primitives, literal-proven immortal `Text` on 64-bit targets, structural tuples, acyclic closed records, fresh-source proven record invariants, established monomorphic refined values, eligible concrete closed enums, and bounded concrete direct generic instances over those representations. Exact type and compile-time witness arguments define deterministic instance identity; regular recursion, duplicate-call elimination, and cross-test-root reuse are supported, while nonregular or over-budget expansion selects atomic fallback before LCIR allocation. Concrete static concept calls close over the selected witness method, conditional prerequisites, and normalized associated bindings, then emit ordinary direct typed calls with no runtime witness. Literal text is one opaque pointer and supports allocation-free length, containment, and content equality without a universal value, GC root, or executor. The broader slice includes tuple/product construction and extraction, record construction, copy/move, bounded typed nested places and functional mutation for invariant-free records, aggregate phis/loops, direct parameters/results, exact transparent refine/unrefine boundaries, proven refined and invariant-product sum payloads, bounded exhaustive match DAGs with shared typed arm blocks, and whole/projected mutable-receiver writeback on normal and fault edges. Checked LCIR has canonical assertion/precondition/postcondition/invariant fault metadata with bounded text and exact contract/blame spans; source contracts and assertions still select atomic fallback until their call-site and cleanup lowering is complete. Complete artifacts use independently checked LCIR and its typed LLVM emitter; only reachable `Unsupported` input selects the complete legacy source graph. The runtime ABI now has a side-by-side precisely described typed moving heap and direct-pointer shadow-root chain, but production LCIR does not yet emit either. Dynamic concept dispatch, portable proof `Recheck`, runtime-checked construction, moving/dynamic text, text in aggregates, other managed values, recursive or open sums, and protected or managed projected inout remain atomic fallback. |
