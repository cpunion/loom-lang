...
默认 `loomc build` 必须产生目标平台 native executable：checked MIR 先经过 root/witness reachability，再 lower 为 LLVM IR、验证、优化并输出 object，最后由平台 linker 链接。`loomc test` 使用同一路径生成 native test harness；`loomc run PATH` 编译临时 native executable 后运行。
