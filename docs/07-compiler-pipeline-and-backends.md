...
当前 C1 LLVM 后端采用混合实现。generic、`dyn`、一般 aggregate 和外部入口仍使用 compiler-private universal `Value` lowering，使 generic function 可以共享 machine body；static concept proof 通过 witness argument 传递。同步、非泛型且参数/结果完全由 primitive scalar 组成的 direct call 已生成私有 typed body 并保留 universal wrapper：`Unit -> i1`、`Bool -> i1`、`Int -> i64`、`Float -> double`。无 invariant、单态、直接 primitive-field 的 POD record 还支持一个有意收窄的跨函数首阶段：eligible direct/inherent producer 以 first-class LLVM aggregate 返回，普通 by-value 参数和 readonly receiver 以 first-class aggregate Value 传入，只有 `mut self` 以 call-scoped InOut pointer 接收 owner。call-site 证明当前只接受 flat `Copy(place)`，也允许 universal caller 从已有 private stack-record storage 直接 pack 参数。closed-world native summary 为 pure/no-fault 时，body 没有 status 或隐藏 context；可能 fault/collect 时返回 `{status, aggregate}` 并接收 context。安全的同步 POD local 把字段节点放入有预算的入口栈存储，release 由 SROA 把热循环字段提升为 SSA；eligible Value/result/InOut call 不物化 managed chain。合同/invariant、nested/managed/generic POD、任意非 flat 参数表达式和 export/unsupported boundary 仍建立当前 universal managed value。同步不逃逸的局部 `List[Int]` 另有 compiler-private contiguous storage。其余 concrete/layout specialization 仍是后续工作，且不得改变 checked overflow、value copy、mutation、ConstraintError 或合同结果。
...
当前 C1 lowering 默认把 generic aggregate、`dyn`、coroutine、外部入口和尚未 specialize 的跨调用 MIR value 放入统一 tag/payload representation。这里称为当前 universal `Value` lowering，而不是对旧布局兼容性的承诺。tag 只帮助现有通用 clone/equality/trace/diagnostic helper 分派，不是语言 RTTI、reflection、stable ABI 或普通值的永久成本。record/enum/refined value 的 payload 由 compiler-managed nodes 表示；copy 做逻辑深拷贝，内部 transfer 可以转移当前表示。上节所述 primitive scalar 私有 ABI、POD aggregate Value 参数/结果、readonly receiver、`mut self` InOut、record scalar projection、局部 POD SROA 和局部 `List[Int]` 快路已绕过对应 envelope/node 成本。aggregate main/test/export、合同/invariant callable、未知 generic、nested/managed record、async frame、动态分派和任意非 flat POD 参数表达式仍保留 universal 表示。POD record 的私有栈节点不会直接逃逸：eligible flat `Copy(place)` 参数、private result 和 InOut call 直接 pack/unpack first-class aggregate，其他 universal boundary 才从当前字段建立独立 managed chain。完整 universal POD body 因而仍负责 fallback 的值复制和 moving-GC 语义，它不是历史 ABI 兼容层；不可达时 release global DCE 会删除它。`Text` 的对象侧表示已经闭合：envelope 只含 tag 与单个 `TextObject*`，长度和 inline UTF-8 位于带 versioned layout descriptor 的对象中，动态对象由 GC 整块移动，字面量使用同布局的 immortal global。该布局只能被 codegen/runtime helpers 观察。
...
最终 typed lowering 以静态类型和 layout descriptor 为依据：concrete scalar、`Text`、record 与已知 generic instance 不需要 per-value tag；enum 只保留自身 variant discriminant；`dyn C` 携带已选 witness/layout proof，但不增加 universal type id。GC trace metadata 可以位于公共 allocation header 或静态 descriptor，它仍不是源码可观察的类型标签。`TextObject` 已闭合对象侧表示，直接 typed local/call ABI 仍须在 generic layout argument、container element layout、coroutine slot layout 与 `dyn` payload layout 完成后移除外围 envelope。`NativeLayout` catalog 已描述 scalar 和无 invariant、单态、直接 primitive-field 的 POD record；record 栈存储与 callable selector 复用该 shape-only 分类。`NativeSignatureShape` 现在放行四种 primitive scalar callable，以及 POD aggregate Value 参数/结果、readonly receiver 与 `mut self` InOut；emitter 再把 native-body closed-world requirement 绑定为 effect-bearing `NativeSignature`，并让声明、调用与 root gate 共用该结果。POD Value 参数和结果使用 compiler-internal first-class LLVM aggregate，由目标后端决定寄存器或机器传递槽，不手工冻结 public `sret`/`byval`；只有 mutable receiver 使用只在该次调用存活的 private InOut pointer。结果 producer 已覆盖 direct/inherent、tail、显式 `return` 和 `if`，fallible status result 保持 fault 传播；参数 source 当前只覆盖 flat `Copy(place)`。只有 selector、独立参数/结果 storage、native/universal requirement 与 GC/逃逸边界同时闭合时才启用，因此“分类为 POD”本身仍不能删除 allocation。带 invariant/requires/ensures 的 callable、nested/managed/generic POD、任意非 flat 参数表达式，以及 List/其他 managed 布局、统一 clone/trace/drop plan 与完整 machine-instance graph 仍待扩展。该优化不改变 checked MIR 或 cache 中的语言语义 identity。
...
`R` 可以是 `i1`/`i64`/`double` 或当前 first-class POD aggregate；native arguments 可以是 primitive scalar 或 first-class POD aggregate Value，只有 mutable POD receiver 使用 InOut pointer。`RuntimeRequirements` 证明为 pure/no-fault 的 native body 及其所选 call edge 没有 status、Runtime、Executor 或其他隐藏 pointer；checked arithmetic 或其他不能证明安全的 body 保留 status，并把 fault/collect 路由到调用方 context。primitive scalar 的 universal wrapper 负责入口拆箱、调用私有 body 和结果装箱；POD 则保留一份完整 universal fallback body，因为 unsupported aggregate boundary 仍需要独立 managed result 和 moving-GC root。当前 POD 参数只从 flat `Copy(place)` pack，readonly receiver 走同一 Value 规则，`mut self` 才走 InOut；合同/invariant 路径仍应采用一次 checked entry 后进入 assumed private body，而不是在热 body 重复 universal 检查。
...
23. eligible POD by-value 参数/readonly receiver/result 使用 first-class aggregate，只有 `mut self` 使用 private InOut pointer；回归覆盖 flat `Copy(place)`、universal caller 的 private stack-record 参数、direct/inherent 与 tail/显式 return/if producer、POD status result 和 fault propagation。universal fallback 保留 unsupported boundary 的独立 managed chain 与 moving-GC root，release 能 DCE 未引用 fallback，基准 hot loop 经 SROA 后没有 node allocation、clone/build 或 root；
