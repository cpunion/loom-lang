...
The production interpreter and native lowering receive only checked MIR. They do
...
`loom-codegen-ir` constructs a complete direct `CheckedArtifact` from an
independently validated `CheckedProgram` and validated roots.
`loom-codegen-llvm` accepts only that checked LCIR artifact. Unsupported
reachable semantics fail native preparation with a structured support report;
there is no checked-MIR LLVM fallback. The exact prepared target and artifact
fingerprint are reused by the production cache and emitter. Broader LCIR
representation and semantic coverage remain tracked by the
