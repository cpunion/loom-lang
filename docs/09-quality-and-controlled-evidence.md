...
5. `NativeLayout` 已统一 scalar 与无 invariant、单态、直接 primitive-field POD record 的 shape-only 分类，现有 record 栈存储候选复用该 catalog；`NativeSignatureShape` 已为 `Unit -> i1`、`Bool -> i1`、`Int -> i64`、`Float -> double` 建立 requirement graph 与 emitter 的共同 private-ABI selector，primitive copy 同样走无分配 scalar path。分类本身不会删除 allocation，只有完整 callable plan 可以。下一步是把当前局部 `List[Int]` fast path 扩展为 managed/generic container layout plan，再补齐 concrete private calling convention、统一 clone/trace/drop plan 和 machine-instance identity；跨函数 POD record ABI 尚未实现，首版计划使用 compiler-internal first-class LLVM aggregate，同时严格分离 typed body 与 universal bridge 的 requirement。随后再以基准和 profiler 证据推进 `Text` direct ABI、known generic instance、含 managed element 的 layout-driven container，以及 coroutine/`dyn` layout descriptor；不得改变值相等、checked overflow、合同、GC tracing 或 concept proof 语义。
...
1. 把现有 primitive-scalar callable plan、函数内 POD record catalog 与局部 `List[Int]` fast path 扩展到跨函数 concrete aggregate call、managed/generic container 与 coroutine ABI，并在有实测收益时增加 hot-site specialization 或单态化；届时再把 canonical type/proof arguments 拆成独立 machine-instance cache entry。
