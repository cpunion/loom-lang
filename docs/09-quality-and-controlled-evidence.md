...
1. 把现有 primitive-scalar callable plan 与函数内 POD record `NativeLayout` catalog 扩展到局部 `List[Int]`、跨函数 concrete aggregate call、managed/generic container 与 coroutine ABI，并在有实测收益时增加 hot-site specialization 或单态化；届时再把 canonical type/proof arguments 拆成独立 machine-instance cache entry。
