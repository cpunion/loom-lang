...
- closed-world callable graph 固定传播独立的 `FAULT`/`COLLECT`/`EXECUTOR` invocation/universal-body/native-body requirements；POD native result 不继承 universal materialization 的 `COLLECT`，root IR 证明普通同步路径不创建 Executor、async 路径建立 Runtime + Executor，runtime fixture 另行证明 reactor/worker 按需创建；
...
- eligible primitive record local 在 development IR 使用有预算的 `record.local` 栈节点；direct/inherent POD producer 以 first-class aggregate 返回，`mut self` 使用 call-scoped InOut pointer 并在 fault status 传播前写回。whole-value copy 及其他 universal boundary 才从当前字段物化独立 managed chain，不让私有栈地址逃逸；universal fallback 保留 balanced root/build，未引用时 release global DCE 删除。POD header/field storage 不进入 shadow roots，普通 loop/return 不生成 GC poll；release golden/benchmark record 热路没有 node traversal、clone/build、root 或 GC allocation，checked accumulator 仍保留；
...
2. concrete scalar、递归 fib 的 checked private ABI 和 POD record 跨函数首阶段已完成：closed-world requirements 让 pure/no-fault scalar call 使用无 status/context 的私有 ABI，fallible path 使用 `{status, value}` 加 context；eligible direct/inherent POD producer 以 first-class aggregate 返回，同类 `mut self` 以 InOut pointer 调用并在 status 传播前写回。native body 不物化 managed result，完整 universal fallback 仍保持普通 aggregate 边界的深复制、root 与 moving-GC 语义；release 可以删除未引用 fallback。record 静态字段读写和局部栈存储仍交给 LLVM SROA，因此 benchmark 热路没有 node allocation/traversal。
...
5. `NativeLayout` 已统一 scalar 与无 invariant、单态、直接 primitive-field POD record 的 shape-only 分类，record 栈存储、requirement graph 与 emitter 复用该 catalog；`NativeSignatureShape` 已为四种 primitive scalar 及 POD result/`mut self` InOut 建立共同 private-ABI selector。POD native/universal body 分开求 summary，first-class aggregate 不冻结 public `sret`/`byval`，分类本身仍不会绕过 storage/GC/逃逸证明。下一步 P0 是普通 by-value POD 参数、readonly receiver，以及带 invariant/requires/ensures callable 的 checked entry + assumed private body；递归 fib 同样应在入口证明范围后进入无逐层 status/context/overflow check 的 assumed body，并保留越界溢出回归。随后把 scalar/POD/List 特判收敛为统一 layout/clone/trace/drop plan 与 machine-instance identity，再按 profiler 证据推进 managed/generic container、`Text` direct ABI、known generic instance 和 coroutine/`dyn` layout descriptor；不得改变值相等、checked overflow、合同、GC tracing 或 concept proof 语义。
...
1. 在 P0 的普通 POD/readonly/contracts 与统一 layout plan 上继续扩展 nested/managed aggregate、generic container 与 coroutine ABI，并在有实测收益时增加 hot-site specialization 或单态化；canonical type/proof arguments 作为独立 machine-instance cache entry，而不是继续堆叠按源码函数共享的 ad-hoc fast path。
